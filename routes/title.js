module.exports = function(app) {
  ...
  api.titles = function (req, res) {
    ...
    Title.find({'TitleName':new RegExp(query,'i')}, 'TitleName TitleId', function(err, titles) {
      if (err) {
        res.json(500, err);
      } else {    
        console.log(titles);
  ...
  api.title = function (req, res) {
    ...
    Title.findOne({ 'TitleId': Number(titleId) }, function(err, title) {
      if (err) {
        res.json(404, err);
      } else {
        console.log('title',title);
        res.json(200, {title: title});
      }
    });
  };
